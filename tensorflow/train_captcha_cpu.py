#!/usr/bin/env python3
"""
train_captcha_cpu.py — CPU-exportable variant of train_captcha.py
==================================================================
與 train_captcha.py 完全相同，唯一差異：
  LSTM 使用 implementation=1（標準 CPU kernel，不使用 CuDNN/Metal）
  -> 產出的 captcha_model_cpu.keras 可以直接轉換為 TFLite

輸出：
  tensorflow/captcha_model_cpu.keras   (可轉 TFLite)
  tensorflow/captcha_model_cpu.h5

執行：
  python train_captcha_cpu.py
  python train_captcha_cpu.py --epochs 300
"""
...
DEFAULT_MODEL_H5     = BASE_DIR / "captcha_model_cpu.h5"
DEFAULT_MODEL_KERAS  = BASE_DIR / "captcha_model_cpu.keras"
