...
**Hard-coded URLs:** `ui/js/api.js` has the GAS URL hard-coded. `gas/BookingEngine.gs` has `WEB_UI_URL` hard-coded. Both must be updated manually and redeployed when the endpoint changes.
...
**CAPTCHA session expiry:** After `runBooking()` emails a captcha, the user has `CONFIG.RETRY_WAIT_MINUTES` (2 min) to submit it via `captcha.html`. After that, the session in ScriptProperties expires and the booking retries from scratch.
